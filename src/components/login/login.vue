     ...
     <div class="login-box">
       ...
       <div>
         <div><input class="login-input"
                     required maxlength="11" v-model.trim="formData.mobile"
                     type="tel"
                     placeholder="请输入手机号"/></div>
         <div><input
           class="login-input" type="password"
           required maxlength="6" v-model.trim="formData.password"
       ...
       <div><router-link to="/restpsd">忘记密码？</router-link></div>
       </div>
       <div class="flex-row justify-between">
...
export default {
  ...
  watch:{
    'formData.password':function(val,oldval){
      val  = encodeURI(this.encryptByDES(val, 'DES_KEY_PASSWORD'))
      this.password = val
    },//
  },
  methods: {
    ...
    confirm () {
      ...
      /*
      * encryptByDES 与 DES_KEY_PASSWORD需要和后台一起确定加密格式
      * */
      // let password
      // if(vm.checked&&localStorage.getItem('password')){
      //   password = localStorage.getItem('password')
      //   // vm.formData.password = localStorage.getItem('password')
      // }else {
      //   password = encodeURI(vm.encryptByDES(vm.formData.password, 'DES_KEY_PASSWORD'))
      // }
